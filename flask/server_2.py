...
async def server(websocket, path):
    ...
    try:
        while True:
            ...
            # str_to_bytearray = message.encode('utf-8')
            # print(f"received: {await message.recv()}")
            data = await websocket.recv()
            data_dict = json.loads(data)
            status = data_dict['status']
            compressed_raw_image = data_dict['image'].encode("latin-1")
            print('type comp: ', type(compressed_raw_image),
                  'len :', len(compressed_raw_image))

            decompress_image = lzma.decompress(compressed_raw_image)
            print('type decomp: ', type(decompress_image),
                  'len :', len(decompress_image))
            str_to_decompress = data.encode("latin-1")
